...
const PointsABI = require("./pointsABI.json");
...
function App() {
  ...
  const pointsContract = () =>
    new ethers.Contract(pointAddress, PointsABI, provider);
  ...
  const createVault = async (vaultName, vaultowner) => {
    try {
      ...
      const result = await contract0
        .connect(signer)
        .createVault(vaultName, vaultowner);
  ...
  const claimPoints = async (_user, _amount) => {
    try {
      const result = await contract1
        .connect(signer)
        .claimPoints(_user, _amount);
      const receipt = await result.wait();
  ...
  const swapToPaymentCoin = async (_user, _amount) => {
    try {
      const result = await contract1
        .connect(signer)
        .swapToPaymentCoin(_user, _amount);
      const receipt = await result.wait();
  ...
  return (
    <div className="">
      ...
      <input
        ...
        className="appearance-none border border-red-500 py-4 pl-4 mb-4  "
      ></input>
      <button
        onClick={() =>
          createVault(name, "0xe050A5B250919d0c552085DF16f71c1716079821")
        }
      ...
      <input
        type="text"
        onChange={(e) => {
          setpointsAmounttoClaim(e.target.value);
        }}
        className="appearance-none border border-red-500 py-4 pl-4 mb-4  "
      ></input>
      <button
        onClick={() =>
          claimPoints(add2, ethers.utils.parseUnits(pointsAmounttoClaim, 18))
        }
        className="p-3 bg-blue-500"
      >
        claimPoints
      ...
      <input
        type="text"
        onChange={(e) => {
          setpointsAmounttoSwap(e.target.value);
        }}
        className="appearance-none border border-red-500 py-4 pl-4 mb-4  "
      ></input>
      <button
        onClick={() =>
          swapToPaymentCoin(
            add2,
            ethers.utils.parseUnits(pointsAmounttoSwap, 18)
          )
        }
        className="p-3 bg-blue-500"
      >

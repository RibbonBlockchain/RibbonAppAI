    ...
    <div className="relative flex flex-col min-h-[100vh] items-start justify-between p-4 sm:p-6">
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="mt-6 mb-4 flex flex-row gap-2 items-center justify-center">
          <p className="text-xs text-[#939393]">0</p>
          <div className="w-full">
            <ProgressBar
              ...
              maxCompleted={data?.question?.length}
            />
          </div>
        </div>

        {data?.questions?.map((q: any, i: any) =>
          i === step ? (
            <>
              <div className="flex flex-col items-center justify-center gap-3">
                <h1 className="font-normal text-lg text-center">{q?.text}</h1>
                {/* <p className="text-[10px] py-1 px-3 bg-[#F6E8F6] rounded-full">
                  {q?.text}
                </p> */}

                <div className="mt-14">
                  <RadioSelect options={q?.options?.map((o: any) => o?.text)} />
                </div>
              </div>
            </>
          ) : (

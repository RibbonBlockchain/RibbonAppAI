...
import { useElectricityPay, useGetDataServiceList } from "@/api/store";
...
const ElectricityBot: React.FC = () => {
  ...
  const [billersCode, setBillersCode] = useState<string>("");

  const { data, error, isLoading } = useGetDataServiceList(
    selectedService || ""
  );
  const { mutate: electricityPay } = useElectricityPay();
  ...
  const handleServiceSelect = (provider: ServiceProvider) => {
    ...
    setMessages((prevMessages) => [
      ...
      {
        sender: "ai",
        text: `You selected ${provider.replace(
          "-data",
          ""
  ...
  const handleVariationSelect = (variationCode: string) => {
    ...
    if (variationCode === "prepaid") {
      setBillersCode("1111111111111");
    } else if (variationCode === "postpaid") {
      setBillersCode("1010101010101");
    }
  ...
  const handlePhoneNumberSubmit = () => {
    ...
    if (/^\d{11}$/.test(phoneNumber)) {
      ...
      setMessages((prevMessages) => [
        ...
        {
          sender: "ai",
          text: `Phone number ${phoneNumber} received. Please enter the amount.`,
        },
      ]);
  ...
  const handleAmountSubmit = () => {
    ...
    setMessages((prevMessages) => [
      ...prevMessages,
      { sender: "user", text: amount },
      {
        sender: "ai",
        text: `Amount ${amount} received. Click on purchase to proceed.`,
      },
    ]);
  ...
  const handleSend = () => {
    ...
    } else if (isAmountInputEnabled && !isPurchasing) {
      handleAmountSubmit();
    } else {
      setMessages((prevMessages) => [
        ...prevMessages,
  ...
  const handlePurchase = async () => {
    if (selectedService && selectedVariation && phoneNumber && amount) {
      setIsPurchasing(true);

      try {
        await electricityPay({
          amount: amount,
          phone: phoneNumber,
  ...
  return (
    <div className="relative w-full mt-2 p-4 flex flex-col h-full overflow-auto scroll-hidden scroll-hidden mx-auto rounded-lg shadow-lg bg-aiBackground bg-contain bg-no-repeat">
      ...
      <div className="self-center bottom-4 w-[90%] max-w-[450px] max-h-[250px] overflow-auto scroll-hidden">
        {isServiceSelectionEnabled && (
          <div className="flex flex-col items-end mb-2">
            ...
            <div className="flex flex-col gap-2">
              {[
                {
                  shortName: "Abuja Electricity Distribution Plc (AEDC)",
                  name: "abuja-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Benin Electricity Distribution Plc (BEDC)",
                  name: "benin-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Eko Electricity Distribution Plc (EKEDC)",
                  name: "eko-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Enugu Electricity Distribution Plc (EEDC)",
                  name: "enugu-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Ibadan Electricity Distribution Plc (IBEDC)",
                  name: "ibadan-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Ikeja Electricity Distribution Plc (IKEDC)",
                  name: "ikeja-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Jos Electricity Distribution Plc (JEDC)",
                  name: "jos-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Kaduna Electricity Distribution Plc (KAEDCO)",
                  name: "kaduna-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                  shortName: "Kano Electricity Distribution Plc (KEDC)",
                  name: "kano-electric",
                  logo: "/assets/sample-icon.png",
                },
                {
                ...
                {
                  shortName: "Yola Electricity Distribution Plc (YEDC)",
                  name: "yola-electric",
                  logo: "/assets/sample-icon.png",
                },
              ].map((provider) => (
                <button
                  ...
                  className="w-full text-start flex flex-row gap-2 items-center px-4 py-2 rounded-[12px] bg-[#3f3952] bg-opacity-95 text-white"
                >
                  <Image
                    width={24}
                    height={24}
        ...
        {isAmountInputEnabled && !isVariationSelectionEnabled && (
          <div className="fixed self-center bottom-4 p-4 w-[90%] max-w-[450px]">
            <div className="relative flex flex-row items-center">
              <input
                type="text"
        ...
        {!isPurchasing && selectedVariation && phoneNumber && amount && (
          <button
